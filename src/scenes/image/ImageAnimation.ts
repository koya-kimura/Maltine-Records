...
export class ImageAnimation {
    ...
    getImage(category: string, animationIndex: number, frameProgress: number): p5.Image {
        ...
        if (!animation || animation.length === 0) {
            throw new Error(`ImageAnimation: animationIndex ${animationIndex} not found in category "${category}" (max: ${categoryAnimations.length - 1})`);
        }
        ...
        if (!img) {
            throw new Error(`ImageAnimation: frame ${frameIndex} not found in animation ${animationIndex} of category "${category}"`);
        }
        return img;
    }
